...
public class SQLs {
    ...
    public static final String PRODUCT_SEARCH =
            "SELECT * FROM product WHERE Name LIKE \"%%%s%%\" AND FinishedGoodsFlag = 1 LIMIT %s, %s";

    //SELECT ProductPhoto using ProductID
    public static final String SELECT_PRODUCT_PHOTO_BY_PRODUCT_ID =
            "SELECT * FROM ProductPhoto LEFT JOIN ProductProductPhoto ON " +
                    "ProductPhoto.ProductPhotoID = ProductProductPhoto.ProductPhotoID WHERE ProductID = ?";

    //COUNT rows in table for SELECT ALL
    public static final String COUNT_ROWS_SELLECT =
            "SELECT COUNT(ProductID) FROM product AND FinishedGoodsFlag = 1";

    //COUNT rows in table for SEARCH
    public static final String COUNT_ROWS_SEARCH =
            "SELECT COUNT(ProductID) FROM product WHERE Name LIKE \"%%%s%%\" AND FinishedGoodsFlag = 1";
}